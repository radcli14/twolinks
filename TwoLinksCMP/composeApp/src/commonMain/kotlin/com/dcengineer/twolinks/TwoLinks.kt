...
class TwoLinks {
    ...
    private val moi: FloatArray
        get() = _moi ?: floatArrayOf(
                1.0f/12.0f * mass[0] * (length[0].pow(2f) + height[0].pow(2f)),
                1.0f/12.0f * mass[1] * (length[1].pow(2f) + height[1].pow(2f))
            )

    private val m11: Float
        get() = _m11 ?: (moi[0] + mass[0] * offset[0].pow(2f) + mass[1] * pivot.pow(2f))

    private val m22: Float
        get() = _m22 ?: (moi[1] + mass[1] * offset[1].pow(2f))
